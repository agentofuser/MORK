        ...
        for dataset in datasets:
            with ins.work_at(dataset) as scope:
                ...
                scope.transform(
                    (
                        "(simple (sister $cid0 $cid1))",
                        "(simple (hasName $cid0 $name0))",
                        "(simple (hasName $cid1 $name1))",
                    ),
                    ("(simple (sisterByName $name0 $name1))",),
                ).block()
                ...
                scope.transform(
                    (
                        "(simple (parent $pid $cid))",
                        "(simple (sister $aid $pid))",
                    ),
                    ("(simple (aunt $aid $cid))",),
                ).block()

                # Add byName aunt relations
                scope.transform(
                    (
                        "(simple (aunt $aid $cid))",
                        "(simple (hasName $aid $name0))",
                        "(simple (hasName $cid $name1))",
                    ),
                    ("(simple (auntByName $name0 $name1))",),
                ).block()
